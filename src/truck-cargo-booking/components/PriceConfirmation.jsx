...
import { calculateLTLPrice } from "../utils/calculateLTLPrice";
import { calculateFTLPrice } from "../utils/calculateFTLPrice";

function money(n) {
  return `$${Number(n || 0).toFixed(2)}`;
}

/** Map Air-style fee rows onto truck LTL/FTL breakdown field names. */
function pickupFee(quote) {
  return quote?.baseFee ?? quote?.vehicleFee ?? 0;
}

function freightFee(quote) {
  return quote?.weightFee ?? quote?.overweightFee ?? 0;
}

function ServiceQuoteCard({ title, quote, deliveryTime, selected, onSelect }) {
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-sm font-medium text-gray-900">{money(pickupFee(quote))}</span>
        </div>
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs text-gray-500">YuuSell Freight</span>
          <span className="text-sm font-medium text-gray-900">{money(freightFee(quote))}</span>
        </div>
        <div className="flex items-center justify-between gap-2">
...
export default function PriceConfirmation({
  ...
  const ltlQuote = calculateLTLPrice(quotePayload);
  const ftlQuote = calculateFTLPrice(quotePayload);
  ...
  const destination =
    (service === "ltl" ? formData?.receiverInfo : formData?.shipperInfo) || "—";
    ...
    <div className="animate-[fadeIn_0.3s_ease-out] max-w-4xl mx-auto flex flex-col gap-5 sm:gap-6">
      <div className="bg-teal-50 border border-teal-100 rounded-2xl px-4 py-3 sm:px-5 sm:py-3.5">
        <p className="text-sm font-semibold text-teal-800 text-center sm:text-left">
          {country}, {fromLocation} → {destination} · {weight}kg · TRUCK
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ServiceQuoteCard
          title="LTL"
          quote={ltlQuote}
          deliveryTime={t("pricing.ltl.deliveryTime", { ns: "truckCargoBooking" })}
          selected={service === "ltl"}
          onSelect={() => onSelectService("ltl")}
        />
        <ServiceQuoteCard
          title="FTL"
          quote={ftlQuote}
          deliveryTime={t("pricing.ftl.deliveryTime", { ns: "truckCargoBooking" })}
          selected={service === "ftl"}
          onSelect={() => onSelectService("ftl")}
        />
      </div>

      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex flex-col gap-3 min-w-0">
        <h3 className="text-sm font-bold text-gray-900">
          {service === "ftl" ? "FTL" : "LTL"} breakdown
        </h3>
        {/* TODO: these labels approximate Zhang Tao's requested breakdown using our existing formula's fields; swap for real carrier-fee fields once a real quote API exists */}
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-gray-500">Origin Pickup</span>
            <span className="text-sm font-medium text-gray-900">{money(pickupFee(breakdown))}</span>
          </div>
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-gray-500">YuuSell Freight</span>
            <span className="text-sm font-medium text-gray-900">{money(freightFee(breakdown))}</span>
          </div>
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-gray-500">Destination Delivery</span>
