...
import { calculateLCLPrice } from "../utils/calculateLCLPrice";
import { calculateFCLPrice } from "../utils/calculateFCLPrice";

function money(n) {
  return `$${Number(n || 0).toFixed(2)}`;
}

/** Map Air-style fee rows onto sea LCL/FCL breakdown field names. */
function pickupFee(quote) {
  return quote?.baseFee ?? quote?.containerFee ?? 0;
}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-sm font-bold text-blue-500 text-right">
          {t(quote.deliveryTimeKey, { ns: "seaCargoBooking" })}
        </span>
      </div>
...
export default function PriceConfirmation({
  ...
  const fclQuote = calculateFCLPrice(quotePayload);
  const lclQuote = calculateLCLPrice(quotePayload);
    ...
    <div className="animate-[fadeIn_0.3s_ease-out] max-w-4xl mx-auto flex flex-col gap-5 sm:gap-6">
      <div className="bg-teal-50 border border-teal-100 rounded-2xl px-4 py-3 sm:px-5 sm:py-3.5">
        <p className="text-sm font-semibold text-teal-800 text-center sm:text-left">
          {fromCountry}, {zipCode} → {destinationCountry} · {weight}kg · SEA
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ServiceQuoteCard
          title="FCL"
          quote={fclQuote}
          selected={service === "fcl"}
          onSelect={() => onSelectService("fcl")}
          t={t}
        />
        <ServiceQuoteCard
          title="LCL"
          quote={lclQuote}
          selected={service === "lcl"}
          onSelect={() => onSelectService("lcl")}
          t={t}
        />
      </div>
